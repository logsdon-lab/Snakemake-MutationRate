...
def main():
    ...
    df_divergence_times = (
        pl.DataFrame(divergence_times)
        .unpivot(variable_name="sm", value_name="divergence_time")
    )
    ...
    df = (
        ...
        .with_columns(
            ...
            mtch_ref=pl.col("ref").str.extract_groups(r"^(?<ref_sm>.*?)~(?<ref_chrom>.*?):(?<ref_st>\d+)-(?<ref_end>\d+)$"),
            qry_sm=pl.col("qry").str.extract(r"^(.*?)~"),
        )
        .unnest("mtch_ref")
        .join(
            df_divergence_times.lazy(),
            left_on=["qry_sm"], right_on=["sm"],
            how="left"
        )
    ...
    df = (
        df.with_columns(
            mu=pl.when(
                pl.col("qry_sm_divergence_time") == 0
            )

...
def main():
    ...
    df = (
        df.sort(by=["chrom", "chrom_st"])
        .with_columns(name=expr_name, item_rgb=expr_item_rgb)
        .with_columns(
            grp=pl.col("chrom").rle_id()
            + pl.col("name").rle_id()
            + pl.col("strand").rle_id(),
        )
        .group_by(["grp"])
        .agg(
            chrom=pl.col("chrom").first(),
            chrom_st=pl.col("chrom_st").min(),
            chrom_end=pl.col("chrom_end").max(),
            name=pl.col("name").first(),
            score=pl.col("score").median(),
            strand=pl.col("strand").first(),
